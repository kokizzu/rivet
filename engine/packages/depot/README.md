...
Per-database storage engine for Rivet's SQLite-on-FDB system. Owns durability (FDB), retention, branching, restore points, and compaction.
...
| `read.rs` + `read/` | `get_pages` planning, PIDX/cache lookup, SHARD fallback |
...
| `shared.rs` | shared planners: `plan_hot_job`, `plan_reclaim_job`, FDB snapshot reads |
...
DbManagerWorkflow         (db_manager.rs)
  ...
  ├─ DbHotCompactorWorkflow   (db_hot_compactor.rs)
...
- **DbHotCompactorWorkflow** — stages hot-shard LTX blobs under `CMP/stage/{job_id}/hot_shard`. Never publishes. The manager installs.
- **DbReclaimerWorkflow** — deletes FDB SHARD/PIDX/COMMIT rows.

### Manager lifecycle

Hot compaction is signal-driven. Reclaim runs on a one-shot deadline that signals re-arm.

| Field | Interval | Behavior |
|---|---|---|
| `next_reclaim_check_at_ms` | `sqlite.manager_reclaim_interval_ms` (10min) | Set to `Some(now+interval)` by any signal when currently `None`. Cleared to `None` when the deadline fires (or a `ForceCompaction(reclaim)` runs). |

The listen waits until that deadline or the next signal. With it `None` the listen blocks until a signal arrives.
...
`manager_effects_after_refresh` only dispatches a planned hot or reclaim job when the matching trigger is set and the `active_jobs.*` slot is empty.

**`schedule_next_wake` rules:**

- A reclaim trigger that fired clears its deadline to `None`.
- If any signal arrived this iteration, a `None` reclaim deadline is armed at `now + interval`. An already-armed deadline is left alone.

**Initial state.** Fresh `DbManagerState::new` has the deadline `None`. The manager parks on a deadline-less listen until the first signal arrives, then arms reclaim for its first cycle.
...
- **`burst_mode.rs`** — hot quota cap. Reads FDB-derived branch lag and decides whether to reject commits. Per-pod stateless.
- **`gc/`** — branch GC pin computation. Refcount + root + desc-pin + restore-point-pin math. Used by hot-history cleanup and debug estimates.
...
Operator scripts. See [`scripts/README.md`](scripts/README.md) for the install and run procedure on an engine pod.

- **`compaction_backlog.py`** — standalone operator script. Reads `META/head` and `CMP/root` per branch straight from FoundationDB to answer how much compaction work is outstanding, and diffs two runs into a drain rate and a projected finish. Byte accounting is sampled and applied to the exact subspace estimate, because a per-branch sizing pass does not scale to a deployment's branch count. No engine build or API access required, and every read is a snapshot read.
- **`check-production-fault-leaks.sh`** — guards that fault-injection points stay behind the `test-faults` feature.
...
CompactionSignaler → unique DbManagerWorkflow (by branch_id tag)
       ...
       ▼
manager refresh → maybe plans hot job → dispatches to DbHotCompactorWorkflow
```

Commit returns durable after FDB commit. RPO = FDB durability.
...
manager RefreshManager activity (FDB snapshot)
       │  computes planned_{hot,reclaim}_job from snapshot + policy
       ▼
manager dispatches RunHotJob / RunReclaimJob signals
       ▼
companion workflow runs activity, signals *JobFinished back
       ▼
manager InstallHotOutput / FinishReclaimJob
...
- `docs-internal/engine/sqlite/storage-structure.md` — exact FDB key layout.
- `docs-internal/engine/sqlite/components.md` — component responsibilities in prose.
- `docs-internal/engine/sqlite/constraints-and-design-decisions.md` — PITR / branching / retention rationale.
