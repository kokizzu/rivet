	use super::{
		BranchState, DBHead, DatabaseBranchId, DatabaseBranchRecord, MetaCompact,
		BucketBranchId, SQLITE_STORAGE_META_VERSION, decode_database_branch_record,
		decode_db_head, decode_meta_compact, encode_database_branch_record, encode_db_head,
		encode_meta_compact,
	};
	...
	fn database_branch_record_rejects_legacy_v1_without_lifecycle_generation() {
		...
		encoded.extend(
			serde_bare::to_vec(&legacy_payload).expect("legacy bare payload should encode"),
		);
