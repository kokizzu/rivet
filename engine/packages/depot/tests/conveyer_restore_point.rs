...
use depot::{
	...
	keys::{
		restore_point_key, branch_commit_key, branch_meta_head_at_fork_key,
		branch_pitr_interval_key, branch_pidx_key, branch_vtx_key, branches_restore_point_pin_key,
		db_pin_key, bucket_branches_pin_count_key,
	},
	conveyer::{restore_point, branch, history_pin},
	pitr_interval::write_pitr_interval_coverage,
	types::{
		DatabaseBranchId, RestorePointRef, RestorePointId, CommitRow, DbHistoryPinKind, DirtyPage, BucketId, PinStatus,
		PitrIntervalCoverage, RestorePointRecord, ResolvedRestoreTarget, ResolvedVersionstamp,
		SnapshotKind, SnapshotSelector, decode_commit_row,
		decode_db_head, decode_db_history_pin, decode_restore_point_record, encode_restore_point_record,
	},
};
...
fn restore_point_new_rejects_malformed_wire_strings() {
	...
	for case in cases {
		assert!(RestorePointId::new(case).is_err(), "{case} should be rejected");
	}
}
...
async fn create_restore_point_returns_retained_restore_point_for_latest_commit() -> Result<()> {
	restore_matrix!("create_restore_point_returns_retained_restore_point_for_latest_commit", |ctx, db, database_db| {

	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let restore_point = database_db.create_restore_point(SnapshotSelector::Latest).await?;

	assert_eq!(restore_point.as_str().len(), 33);
	assert_eq!(restore_point.parse()?, (1_000, 1));
	assert!(
		common::read_value(&db, restore_point_key(TEST_DATABASE, restore_point.as_str()))
			.await?
			.is_some()
	);
...
async fn create_restore_point_from_timestamp_selector_pins_selected_interval() -> Result<()> {
	restore_matrix!("create_restore_point_from_timestamp_selector_pins_selected_interval", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	...
	let restore_point = database_db
		.create_restore_point(SnapshotSelector::AtTimestamp { timestamp_ms: 1_500 })
		.await?;

	assert_eq!(restore_point.parse()?, (1_000, 1));
	let record_bytes = common::read_value(&db, restore_point_key(TEST_DATABASE, restore_point.as_str()))
		.await?
	...
	let pin_bytes = common::read_value(
		&db,
		db_pin_key(branch_id, &history_pin::restore_point_pin_id(&restore_point)),
	)
	.await?
...
async fn create_restore_point_revalidates_target_commit_after_resolve_race() -> Result<()> {
	restore_matrix!("create_restore_point_revalidates_target_commit_after_resolve_race", |ctx, db, _database_db| {
	...
	let (_guard, reached, release) = restore_point::test_hooks::pause_after_resolve(database_id);
	let create_task = {
		let db = db.clone();
...
async fn restore_point_status_reads_pinned_record_or_absent() -> Result<()> {
	restore_matrix!("restore_point_status_reads_pinned_record_or_absent", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let restore_point = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	...
	let database_branch_id = db
		.run({
			...
			move |tx| {
				...
				async move {
					let branch_id =
						branch::resolve_database_branch(&tx, bucket_id, TEST_DATABASE, Serializable)
							.await?
...
async fn restore_point_ready_transition_writes_history_pin_without_object_key() -> Result<()> {
	restore_matrix!("restore_point_ready_transition_writes_history_pin_without_object_key", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
	...
	let pin_count = common::read_value(&db, bucket_branches_pin_count_key(bucket_branch_id))
		.await?
...
async fn legacy_failed_restore_point_status_preserves_object_key() -> Result<()> {
	restore_matrix!("legacy_failed_restore_point_status_preserves_object_key", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
	let row = commit_row(&db, branch_id, 1).await?;
	...
	assert_eq!(
		database_db.restore_point_status(restore_point.clone()).await?,
		Some(PinStatus::Failed)
	);
	let pinned_bytes = common::read_value(&db, restore_point_key(TEST_DATABASE, restore_point.as_str()))
		.await?
	...
	assert_eq!(
		common::read_value(&db, db_pin_key(branch_id, &history_pin::restore_point_pin_id(&restore_point))).await?,
		None
	);
...
async fn create_restore_point_enforces_bucket_pin_cap() -> Result<()> {
	restore_matrix!("create_restore_point_enforces_bucket_pin_cap", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let bucket_branch_id = bucket_branch_id(&db, bucket_id).await?;
	db.run(move |tx| async move {
...
async fn delete_restore_point_removes_pin_and_recomputes_branch_pin() -> Result<()> {
	restore_matrix!("delete_restore_point_removes_pin_and_recomputes_branch_pin", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let first = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
	let bucket_branch_id = bucket_branch_id(&db, bucket_id).await?;
	...
	let second = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	...
	assert_eq!(
		common::read_value(&db, db_pin_key(branch_id, &history_pin::restore_point_pin_id(&first))).await?,
		None
	);
	assert!(
	...
	assert!(
		common::read_value(&db, db_pin_key(branch_id, &history_pin::restore_point_pin_id(&second)))
			.await?
			.is_some()
	);
	assert_eq!(
...
async fn deleting_last_restore_point_clears_branch_pin_for_later_pin() -> Result<()> {
	restore_matrix!("deleting_last_restore_point_clears_branch_pin_for_later_pin", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let first = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
...
async fn restore_database_from_restore_point_rolls_back_then_pins_undo_restore_point() -> Result<()> {
	restore_matrix!("restore_database_from_restore_point_rolls_back_then_pins_undo_restore_point", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let target = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	database_db
	...
	let undo = database_db
		.restore_database(SnapshotSelector::RestorePoint { restore_point: target })
		.await?;
	...
	let restored_head_bytes = common::read_value(&db, branch_meta_head_at_fork_key(new_branch_id))
		.await?
	...
	let pinned_bytes = common::read_value(&db, restore_point_key(TEST_DATABASE, undo.as_str()))
		.await?
...
async fn restore_database_from_timestamp_rolls_back_then_pins_undo_restore_point() -> Result<()> {
	restore_matrix!("restore_database_from_timestamp_rolls_back_then_pins_undo_restore_point", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	database_db
		.commit(vec![page(1, 0x22), page(2, 0x33)], 3, 2_000)
		.await?;
	...
	let undo = database_db
		.restore_database(SnapshotSelector::AtTimestamp { timestamp_ms: 1_500 })
		.await?;
...
async fn restore_database_rollback_and_undo_pin_are_atomic() -> Result<()> {
	restore_matrix!("restore_database_rollback_and_undo_pin_are_atomic", |ctx, db, _database_db| {
	...
	assert!(
		err.chain()
			.any(|cause| cause.to_string().contains("injected failure after sqlite restore rollback"))
	);
	assert_eq!(
...
async fn write_after_pitr_restore_lands_on_restored_branch() -> Result<()> {
	restore_matrix!("write_after_pitr_restore_lands_on_restored_branch", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	database_db
		.commit(vec![page(1, 0x22), page(2, 0x33)], 3, 2_000)
		.await?;
...
async fn restore_database_rejects_expired_timestamp_without_undo_or_pointer_swap() -> Result<()> {
	restore_matrix!("restore_database_rejects_expired_timestamp_without_undo_or_pointer_swap", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let old_branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
...
async fn resolve_restore_point_uses_retained_record_without_vtx() -> Result<()> {
	restore_matrix!("resolve_restore_point_uses_retained_record_without_vtx", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let restore_point = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
	...
	clear_value(&db, restore_point_key(TEST_DATABASE, restore_point.as_str())).await?;
	let err = database_db
...
async fn resolve_restore_target_latest_returns_current_head_metadata() -> Result<()> {
	restore_matrix!("resolve_restore_target_latest_returns_current_head_metadata", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
...
#[tokio::test]
async fn resolve_restore_target_timestamp_uses_latest_unexpired_interval_before_target() -> Result<()> {
	restore_matrix!("resolve_restore_target_timestamp_uses_latest_unexpired_interval_before_target", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	database_db.commit(vec![page(1, 0x22)], 2, 2_500).await?;
	...
	let between = database_db
		.resolve_restore_target(SnapshotSelector::AtTimestamp { timestamp_ms: 2_750 })
		.await?;
	...
	let quiet_period = database_db
		.resolve_restore_target(SnapshotSelector::AtTimestamp { timestamp_ms: 2_999 })
		.await?;
	assert_eq!(quiet_period.txid, 2);

	let walked_back = database_db
		.resolve_restore_target(SnapshotSelector::AtTimestamp { timestamp_ms: 2_100 })
		.await?;
...
async fn resolve_restore_target_timestamp_rejects_expired_interval_coverage() -> Result<()> {
	restore_matrix!("resolve_restore_target_timestamp_rejects_expired_interval_coverage", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
	let row = commit_row(&db, branch_id, 1).await?;
	...
	let err = database_db
		.resolve_restore_target(SnapshotSelector::AtTimestamp { timestamp_ms: 1_000 })
		.await
...
async fn restore_point_survives_after_source_interval_coverage_expires() -> Result<()> {
	restore_matrix!("restore_point_survives_after_source_interval_coverage_expires", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
	let row = commit_row(&db, branch_id, 1).await?;
	...
	.await?;
	let restore_point = database_db
		.create_restore_point(SnapshotSelector::AtTimestamp { timestamp_ms: 1_000 })
		.await?;
...
async fn resolve_restore_target_restore_point_returns_exact_metadata() -> Result<()> {
	restore_matrix!("resolve_restore_target_restore_point_returns_exact_metadata", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let restore_point = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
...
async fn resolve_restore_target_restore_point_rejects_missing_record() -> Result<()> {
	restore_matrix!("resolve_restore_target_restore_point_rejects_missing_record", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
...
async fn resolve_restore_point_prefers_exact_pinned_record() -> Result<()> {
	restore_matrix!("resolve_restore_point_prefers_exact_pinned_record", |ctx, db, database_db| {
	database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
	let restore_point = database_db.create_restore_point(SnapshotSelector::Latest).await?;
	let bucket_id = BucketId::from_gas_id(test_bucket());
	let branch_id = database_branch_id(&db, bucket_id, TEST_DATABASE).await?;
...
async fn resolve_restore_point_rejects_missing_record_on_forked_database() -> Result<()> {
	restore_matrix!("resolve_restore_point_rejects_missing_record_on_forked_database", |ctx, db, source_database_db| {
		source_database_db
			.commit(vec![page(1, 0x11)], 2, 1_000)
			.await?;
		let restore_point = source_database_db.create_restore_point(SnapshotSelector::Latest).await?;
		let bucket_id = BucketId::from_gas_id(test_bucket());
		...
		let err = restore_point::resolve_restore_point(&db, bucket_id, forked_database_id, restore_point)
			.await
...
async fn resolve_restore_point_honors_bucket_fork_versionstamp_cap() -> Result<()> {
	restore_matrix!("resolve_restore_point_honors_bucket_fork_versionstamp_cap", |ctx, db, source_database_db| {
		source_database_db
			.commit(vec![page(1, 0x11)], 2, 1_000)
			.await?;
		let bucket_id = BucketId::from_gas_id(test_bucket());
		...
		source_database_db
			...
			.await?;
		let post_fork_restore_point = source_database_db.create_restore_point(SnapshotSelector::Latest).await?;
