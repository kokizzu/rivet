use depot::conveyer::keys::{
	...
	BUCKET_PTR_PARTITION, CMPC_PARTITION, CTR_PARTITION, CompactorQueueKind,
	DB_BRANCH_OWNER_PARTITION, DB_PIN_PARTITION, DBPTR_PARTITION, PAGE_SIZE,
	...
	ctr_quota_global_key, database_branch_owner_key, database_pitr_policy_key,
...
#[test]
fn database_branch_owner_key_is_branch_scoped() {
	let branch = database_branch_id();
	let mut expected = vec![SQLITE_SUBSPACE_PREFIX, DB_BRANCH_OWNER_PARTITION, b'/'];
	expected.extend_from_slice(&uuid_bytes(branch.as_uuid()));
	assert_eq!(database_branch_owner_key(branch), expected);
}
...
fn database_branch_data_keys_live_under_br_partition() {
	...
	// What `branch_range` has to guarantee is that a scan over it reaches every branch-local key.
	// Asserting its exact byte shape instead would only restate the subspace library's internals, and
	// would keep passing if a key were added that the range does not cover.
	for key in &keys {
		assert!(key.starts_with(&prefix), "key escapes the branch prefix");
		assert!(
			start.as_slice() <= key.as_slice() && key.as_slice() < end.as_slice(),
			"key falls outside the branch range"
		);
	}
