...
use depot::{
	...
	types::{
		DatabaseBranchId, ColdManifestChunk, ColdManifestChunkRef, ColdManifestIndex,
...
async fn debug_dumps_ancestry_pins_restore_points_and_gc_pin() -> Result<()> {
	common::test_matrix("depot-debug-ancestry", |_tier, ctx| Box::pin(async move {
		...
		let pinned = database_db.create_restore_point(depot::types::SnapshotSelector::Latest).await?;
		...
		assert!(restore_points.iter().any(|entry| {
			entry.restore_point_id == pinned && entry.pin_status == depot::types::PinStatus::Ready
		}));
...
async fn debug_read_at_returns_page_state_for_versionstamp() -> Result<()> {
	common::test_matrix("depot-debug-read-at", |_tier, ctx| Box::pin(async move {
		let db = ctx.udb.clone();
		let database_db = ctx.db;
...
async fn debug_dump_cold_manifest_reads_index_and_chunks() -> Result<()> {
	common::test_matrix("depot-debug-cold-manifest", |tier, ctx| Box::pin(async move {
		let database_db = ctx.db;
		database_db.commit(vec![page(1, 0x11)], 2, 1_000).await?;
		let branch_id = debug::dump_database_ancestry(&database_db).await?[0].0;
