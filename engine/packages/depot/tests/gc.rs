...
use depot::{
	...
	keys::{
		...
		branch_shard_key, branch_vtx_key, branches_restore_point_pin_key, branches_desc_pin_key,
		branches_list_key, branches_refcount_key, db_pin_key,
	},
	types::{
		DatabaseBranchId, DatabaseBranchRecord, BranchState, CommitRow, BucketBranchId,
...
async fn sweeping_child_branch_releases_parent_refcount_and_fork_pin() -> Result<()> {
	common::test_matrix("depot-gc-child-branch", |_tier, ctx| Box::pin(async move {
		...
		db.run(move |tx| async move {
			...
			tx.informal()
				.set(&branches_refcount_key(parent_branch_id), &2_i64.to_le_bytes());
			tx.informal()
			...
			tx.informal()
				.set(&branches_refcount_key(child_branch_id), &0_i64.to_le_bytes());
		...
		assert_eq!(read_value(&db, branches_list_key(child_branch_id)).await?, None);
...
async fn branch_gc_pin_recomputes_from_current_counters_without_ratchet() -> Result<()> {
	common::test_matrix("depot-gc-pin-recompute", |_tier, ctx| Box::pin(async move {
		let db = ctx.udb.clone();
		let branch_id = branch_id();
		...
		db.run(move |tx| async move {
			...
			tx.informal().set(&branches_restore_point_pin_key(branch_id), &[8; 16]);
			Ok(())
		})
		.await?;
		...
		db.run(move |tx| async move {
			...
			tx.informal()
				.set(&branches_restore_point_pin_key(branch_id), &VERSIONSTAMP_INFINITY);
			Ok(())
		})
		.await?;
		let pin = estimate_branch_gc_pin(&db, branch_id)
...
async fn unreferenced_unpinned_branch_sweep_deletes_hot_branch_data() -> Result<()> {
	common::test_matrix("depot-gc-unreferenced-sweep", |_tier, ctx| Box::pin(async move {
		let db = ctx.udb.clone();
		let branch_id = branch_id();
		...
		db.run(move |tx| async move {
			tx.informal()
				.set(&branches_desc_pin_key(branch_id), &VERSIONSTAMP_INFINITY);
			tx.informal().set(&branches_restore_point_pin_key(branch_id), &VERSIONSTAMP_INFINITY);
			tx.informal()
		...
		assert_eq!(read_value(&db, branches_restore_point_pin_key(branch_id)).await?, None);
...
async fn branch_hot_gc_uses_vtx_floor_for_commits_vtx_and_delta() -> Result<()> {
	common::test_matrix("depot-gc-hot-history", |_tier, ctx| Box::pin(async move {
		let db = ctx.udb.clone();
		let branch_id = branch_id();
