...
use depot::{
	...
	keys::{
		branch_commit_key, branch_meta_head_at_fork_key, branch_vtx_key, branches_restore_point_pin_key,
		branches_desc_pin_key,
	},
	conveyer::branch,
	types::{DatabaseBranchId, DirtyPage, BucketBranchId, BucketId, decode_commit_row},
};
...
async fn gc_pin_recompute_under_restore_point_delete_race() -> Result<()> {
	common::test_matrix("depot-gc-restore-point-race", |_tier, ctx| Box::pin(async move {
		...
		db.run(move |tx| async move {
			...
			tx.informal().clear(&branches_restore_point_pin_key(branch_id));
			Ok(())
		})
		.await?;
