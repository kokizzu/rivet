//! vbare version wrappers for Actor Runtime Socket frames.
		...
		impl OwnedVersionedData for $name {
			...
			fn deserialize_version(payload: &[u8], version: u16) -> Result<Self> {
				match version {
					...
					_ => bail!("unsupported Actor Runtime Socket protocol version: {version}"),
				}
			}
			...
			fn serialize_version(self, version: u16) -> Result<Vec<u8>> {
				match (self, version) {
					...
					(_, version) => {
						bail!("unsupported Actor Runtime Socket protocol version: {version}")
					}
				}
			}
