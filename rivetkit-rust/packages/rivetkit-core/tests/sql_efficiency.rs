...
fn query_catalog() -> Vec<QueryCase> {
	...
	vec![
		...
		QueryCase { id: "queue.receive", sql: internal_storage::load_queue_messages_matching_sql(0), params: vec![5_i64.into()], expectation: bounded_scan(&[AllowedScan { table: "_rivet_queue", reason: "queue receive scans primary-key order only until it reaches the requested count", bound: "LIMIT caps returned message bodies; ActorConfig.max_queue_size caps the no-match scan" }]) },
		QueryCase { id: "queue.receive_names", sql: internal_storage::load_queue_messages_matching_sql(2), params: vec![text("message"), text("other"), 5_i64.into()], expectation: bounded_scan(&[AllowedScan { table: "_rivet_queue", reason: "filtered queue receive preserves global enqueue order without a write-amplifying name index", bound: "LIMIT caps returned message bodies; ActorConfig.max_queue_size caps the no-match scan" }]) },
