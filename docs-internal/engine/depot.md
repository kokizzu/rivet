...
Depot stores SQLite pages in UDB. UDB is the durable source of truth.
...
| `BR/{branch}/SHARD` | Reader-visible shard versions | Workflow manager, reclaimer |
| `BR/{branch}/CMP/*` | Workflow manifest and staged hot output | Workflow manager and companions |
...
A branch carried over from an enterprise build can still hold `CMP/cold_shard` rows whose objects are unreachable here. If only such coverage could satisfy a page, reads fail with `ShardCoverageMissing` instead of inventing zero-filled bytes.
...
The commit path does **not** publish SHARD rows or delete old history. It only records new committed history and wakes workflow compaction.
...
- Reclaim jobs delete hot rows only after the manager proves replacement coverage.
