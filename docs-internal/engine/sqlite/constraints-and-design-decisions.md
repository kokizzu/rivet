...
- **Lazy reads.** Forks do not copy data. Reads walk branch ancestry and hydrate from FDB only when needed.
...
LTX layers carry page numbers and checksums. That lets the system move bytes between DELTA and SHARD layers without a separate opaque page map inside the object. FDB PIDX remains the routing index.
...
Reads choose the largest `as_of_txid <= read_txid`. Reclaim deletes old versions only once a newer version supersedes them and descendant/restore_point pins allow it.
